...
declared inputs by sha256, profiles the inputs, computes the plan's declared
metrics, and writes results.json plus audit.md. It intentionally does not
install packages, download code, or call network APIs.

Metrics are declarative: every aggregation is named in analysis-plan.json and
executed by the engine below. Nothing in the plan is evaluated as code.
"""
from __future__ import annotations
...
# ---------------------------------------------------------------------------
# Metric engine. Specs are declarative data from analysis-plan.json.
# ---------------------------------------------------------------------------

TRUTHY_DEFAULT = ("1", "1.0", "true", "t", "yes", "y", "error")


class MetricError(Exception):
    """A metric spec that cannot be executed against the declared input."""


def is_truthy(value: Any, truthy: tuple[str, ...]) -> bool:
    if value is None:
        return False
    if isinstance(value, bool):
        return value
...
def numeric_values(rows: list[dict[str, Any]], column: str) -> list[Decimal]:
    out = [d for d in (decimal_or_none(r.get(column)) for r in rows) if d is not None]
    out.sort()
    return out


def percentile(sorted_values: list[Decimal], q: float) -> Decimal | None:
    """Nearest-rank percentile (rank = ceil(q * n)) on a pre-sorted list."""
    if not sorted_values:
        return None
    rank = int(math.ceil(float(q) * len(sorted_values)))
    rank = min(max(rank, 1), len(sorted_values))
    return sorted_values[rank - 1]


def number_out(value: Decimal | None) -> Any:
    if value is None:
        return None
...
def bucket_value(value: Any, unit: str) -> str | None:
    text = str(value or "").strip()
    if not text:
        return None
    parsed = None
    for candidate in (text.replace("Z", "+00:00"), text[:19], text[:10]):
        try:
    ...
    raise MetricError(f"unsupported bucket unit: {unit}")


def compare_numeric(left: Any, op: str, right: Any) -> bool:
...
def passes_filters(row: dict[str, Any], filters: list[dict[str, Any]]) -> bool:
    for f in filters:
        col = f.get("column")
        op = f.get("op") or "eq"
        raw = row.get(col)
        ...
        if op == "nonblank":
            ...
            if text != str(f.get("value")):
                return False
        elif op == "ne":
            if text == str(f.get("value")):
                return False
        elif op == "in":
            if text not in [str(v) for v in (f.get("values") or [])]:
                return False
        elif op == "not_in":
            if text in [str(v) for v in (f.get("values") or [])]:
                return False
        elif op == "contains":
            if str(f.get("value")).lower() not in text.lower():
                return False
        elif op in ("gt", "gte", "lt", "lte"):
            if not compare_numeric(raw, op, f.get("value")):
                return False
        else:
            raise MetricError(f"unsupported filter op: {op}")
    return True
...
def aggregate(
    ...
    for agg in aggregations:
        op = agg.get("op")
        col = agg.get("column")
        name = agg.get("name") or (op if not col else f"{op}_{col}")
        if op == "count":
            out[name] = len(rows)
            continue
        if op == "share":
            out[name] = round(100.0 * len(rows) / denominator_rows, 4) if denominator_rows else None
            continue
        if op == "count_distinct":
            ...
            continue
        if op == "rate":
            truthy = tuple(str(v).strip().lower() for v in (agg.get("truthy") or TRUTHY_DEFAULT))
            nonblank_only = agg.get("denominator") == "nonblank"
            pool = [r for r in rows if str(r.get(col) or "").strip() != ""] if nonblank_only else rows
            hits = sum(1 for r in pool if is_truthy(r.get(col), truthy))
            out[name] = round(100.0 * hits / len(pool), 4) if pool else None
            if nonblank_only:
                # The denominator differs from the group's row count, so show it.
                out[f"{name}_n"] = len(pool)
            coverage[col] = len(pool)
            continue
        values = numeric_values(rows, col)
        coverage[col] = len(values)
        if op == "sum":
            out[name] = number_out(sum(values, Decimal("0"))) if values else None
        elif op == "mean":
            out[name] = number_out(sum(values, Decimal("0")) / Decimal(len(values))) if values else None
        elif op == "min":
            out[name] = number_out(values[0]) if values else None
        elif op == "max":
            out[name] = number_out(values[-1]) if values else None
        elif op == "median":
            out[name] = number_out(percentile(values, 0.5))
        elif op == "percentile":
            out[name] = number_out(percentile(values, float(agg.get("q") or 0.5)))
        else:
            raise MetricError(f"unsupported aggregation op: {op}")
    return out, coverage


def known_columns(rows: list[dict[str, Any]]) -> set[str]:
    # Scans every row: JSON/JSONL inputs may be ragged, and a sampled scan would
    # reject a column that only appears later in the file.
    cols: set[str] = set()
    for row in rows:
...
def compute_metric(spec: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    filters = spec.get("filter") or []
    if isinstance(filters, dict):
        filters = [filters]
    group_by = [c for c in (spec.get("group_by") or []) if c]
    bucket = spec.get("bucket") or None
    aggregations = spec.get("aggregations") or [{"name": "count", "op": "count"}]

    referenced = set(group_by)
    for agg in aggregations:
        if agg.get("column"):
            referenced.add(agg["column"])
    for f in filters:
        if f.get("column"):
            referenced.add(f["column"])
    if bucket and bucket.get("column"):
        referenced.add(bucket["column"])
    present = known_columns(rows)
    if present:
        missing = sorted(c for c in referenced if c not in present)
        if missing:
            raise MetricError("column(s) not present in input: " + ", ".join(missing))

    scoped = [r for r in rows if passes_filters(r, filters)]

    key_fields = list(group_by)
    if bucket:
        key_fields.append(bucket.get("as") or f"{bucket.get('column')}_{bucket.get('unit') or 'day'}")
    ...
    for row in scoped:
        ...
        if bucket:
            b = bucket_value(row.get(bucket["column"]), bucket.get("unit") or "day")
            if b is None:
                unbucketable += 1
                continue
            parts.append(b)
    ...
    groups = []
    for key in order:
        values, _ = aggregate(grouped[key], aggregations, len(scoped))
        entry = {"key": dict(zip(key_fields, key))}
        entry.update(values)
        groups.append(entry)
    ...
    if sort_by is None:
        for agg in aggregations:
            if agg.get("op") == "count":
                sort_by = agg.get("name") or "count"
                break
    if sort_by and key_fields:
        descending = spec.get("sort_desc")
        descending = True if descending is None else bool(descending)
        if sort_by in key_fields:
            groups.sort(key=lambda g: str(g["key"].get(sort_by) or ""), reverse=descending)
        else:
            groups.sort(
                key=lambda g: (g.get(sort_by) is None, g.get(sort_by) if g.get(sort_by) is not None else 0),
                reverse=descending,
            )
    elif key_fields:
        groups.sort(key=lambda g: tuple(str(v) for v in g["key"].values()))

    top_n = spec.get("top_n")
    truncated = None
    if top_n and len(groups) > int(top_n):
        truncated = int(top_n)
        groups = groups[: int(top_n)]

    totals, coverage = aggregate(scoped, aggregations, len(scoped))
    out: dict[str, Any] = {
        "name": spec.get("name"),
        "group_by": key_fields,
    ...
    if truncated is not None:
        out["truncated_to"] = truncated
    if unbucketable:
        out["rows_dropped_unparseable_bucket"] = unbucketable
    return out
...
def rows_for_spec(
    ...
    target = spec.get("input")
    if target:
        wanted = str(Path(target).expanduser().resolve())
        if wanted not in inputs_by_path:
            matches = [p for p in inputs_by_path if Path(p).name == Path(target).name]
            if len(matches) != 1:
                raise MetricError(f"metric input not declared in plan: {target}")
            wanted = matches[0]
    else:
        if not inputs_by_path:
            raise MetricError("plan declares no inputs")
        wanted = next(iter(inputs_by_path))
    inp = inputs_by_path[wanted]
    ...
    if input_type == "sqlite":
        if not table:
            raise MetricError("metric on a sqlite input requires a 'table' field")
        conn = sqlite3.connect(f"file:{wanted}?mode=ro", uri=True)
        conn.row_factory = sqlite3.Row
        ...
        if table not in names:
            conn.close()
            raise MetricError(f"table not found in database: {table}")
        quoted = '"' + table.replace('"', '""') + '"'
        rows = [dict(r) for r in conn.execute(f"SELECT * FROM {quoted}")]  # nosec: identifier validated against sqlite_master
        conn.close()
    else:
        rows = read_rows(Path(wanted), input_type)
    cache[key] = rows
    return rows
...
def compute_metrics(
    ...
    for index, spec in enumerate(specs):
        name = spec.get("name") or f"metric_{index + 1}"
        try:
            result = compute_metric({**spec, "name": name}, rows_for_spec(spec, inputs_by_path, cache))
        except (MetricError, OSError, ValueError, sqlite3.Error, json.JSONDecodeError, csv.Error) as e:
...
def metrics_markdown(metrics: list[dict[str, Any]]) -> list[str]:
    if not metrics:
        return []
    lines = ["\n## Metrics\n\n"]
    for m in metrics:
        ...
        if m.get("error"):
            lines.append(f"- **not computed** — {m['error']}\n\n")
            continue
        notes = [f"rows scoped: {m['rows_scoped']}", f"groups: {m['group_count']}"]
        if m.get("truncated_to"):
            notes.append(f"showing top {m['truncated_to']}")
        if m.get("rows_dropped_unparseable_bucket"):
            notes.append(f"rows with unparseable bucket key: {m['rows_dropped_unparseable_bucket']}")
        lines.append(" · ".join(notes) + "\n\n")
        groups = m.get("groups") or []
        if not groups:
            lines.append("- no rows matched\n\n")
            continue
        key_fields = m.get("group_by") or []
        agg_fields = [k for k in groups[0] if k != "key"]
        headers = key_fields + agg_fields
        table_rows = []
        for g in groups:
            table_rows.append([g["key"].get(k) for k in key_fields] + [g.get(k) for k in agg_fields])
        if key_fields:
            table_rows.append(["ALL"] * len(key_fields) + [m["totals"].get(k) for k in agg_fields])
        lines += markdown_table(headers or agg_fields, table_rows)
    return lines
...
def main() -> int:
    ...
    metrics = compute_metrics(plan.get("metrics") or [], inputs_by_path)
    for m in metrics:
        if m.get("error"):
            validations.append({
                "name": f"metric:{m.get('name')}",
                "passed": False,
                "expected": "metric computes against the declared input",
                "actual": m["error"],
            })
    ...
    lines += metrics_markdown(metrics)
    lines.append("\n## Validations\n\n")
...
# ---------- analysis metric specs ----------

_METRIC_AGG_OPS = {
    "count", "share", "count_distinct", "rate",
    "sum", "mean", "min", "max", "median", "percentile",
}
_METRIC_AGG_OPS_NEEDING_COLUMN = {"count_distinct", "rate", "sum", "mean", "min", "max", "median", "percentile"}
_METRIC_FILTER_OPS = {"eq", "ne", "in", "not_in", "contains", "gt", "gte", "lt", "lte", "blank", "nonblank"}
_METRIC_BUCKET_UNITS = {"hour", "day", "week", "month", "year"}
...
def _ident_tokens(text: str) -> list[str]:
    return [_singular(t) for t in _norm_ident(text).split("_") if t]


def _match_column(term: str, columns: list[str]) -> str | None:
    """Map a question phrase to a real column name, or None.

    Multi-word phrases may match by token containment ("integration surface" ->
    `surface`). Single-word phrases must match a column identically, which stops
    a bare "error" from being read as a grouping dimension.
    """
    term_norm = _norm_ident(term)
    if not term_norm:
        return None
    by_norm = {_norm_ident(c): c for c in columns}
    if term_norm in by_norm:
        return by_norm[term_norm]
    term_tokens = _ident_tokens(term)
    for col in columns:
        if _ident_tokens(col) == term_tokens:
            return col
    ...
    for col in columns:
        col_tokens = set(_ident_tokens(col))
        if col_tokens and col_tokens <= term_set:
            score = len(col_tokens)
            if best is None or score > best[0]:
                best = (score, col)
    return best[1] if best else None


def _normalize_metric_specs(raw: Any) -> list[dict[str, Any]]:
    """Validate a declarative metric spec list. Raises ValueError on bad input."""
    if raw is None:
        return []
    if isinstance(raw, dict):
        raw = raw.get("metrics", raw)
        if isinstance(raw, dict):
            raw = [raw]
    ...
    for index, item in enumerate(raw):
        ...
        spec: dict[str, Any] = {"name": str(item.get("name") or f"metric_{index + 1}")}
        label = f"metric '{spec['name']}'"
        for key in ("input", "table", "description", "sort_by"):
        ...
        if not isinstance(group_by, list):
            raise ValueError(f"{label}: group_by must be a string or list of strings")
        spec["group_by"] = [str(c) for c in group_by]
        bucket = item.get("bucket")
        if bucket is not None:
            if not isinstance(bucket, dict) or not bucket.get("column"):
                raise ValueError(f"{label}: bucket must be an object with a 'column'")
            unit = str(bucket.get("unit") or "day")
            if unit not in _METRIC_BUCKET_UNITS:
                raise ValueError(f"{label}: bucket unit must be one of {sorted(_METRIC_BUCKET_UNITS)}")
            spec["bucket"] = {"column": str(bucket["column"]), "unit": unit}
            if bucket.get("as"):
                spec["bucket"]["as"] = str(bucket["as"])
        filters = item.get("filter") or []
        if isinstance(filters, dict):
            filters = [filters]
        if not isinstance(filters, list):
            raise ValueError(f"{label}: filter must be an object or list of objects")
        norm_filters = []
        for f in filters:
            if not isinstance(f, dict) or not f.get("column"):
                raise ValueError(f"{label}: each filter needs a 'column'")
            op = str(f.get("op") or "eq")
            if op not in _METRIC_FILTER_OPS:
                raise ValueError(f"{label}: filter op '{op}' must be one of {sorted(_METRIC_FILTER_OPS)}")
            entry: dict[str, Any] = {"column": str(f["column"]), "op": op}
            if op in ("in", "not_in"):
                values = f.get("values")
                if not isinstance(values, list) or not values:
                    raise ValueError(f"{label}: filter op '{op}' requires a non-empty 'values' list")
                entry["values"] = list(values)
            elif op not in ("blank", "nonblank"):
                if "value" not in f:
                    raise ValueError(f"{label}: filter op '{op}' requires a 'value'")
                entry["value"] = f["value"]
            norm_filters.append(entry)
        if norm_filters:
            spec["filter"] = norm_filters
        aggregations = item.get("aggregations") or [{"name": "count", "op": "count"}]
        if not isinstance(aggregations, list) or not aggregations:
            raise ValueError(f"{label}: aggregations must be a non-empty list")
        norm_aggs = []
        for agg in aggregations:
            if not isinstance(agg, dict):
                raise ValueError(f"{label}: each aggregation must be an object")
            op = str(agg.get("op") or "")
            if op not in _METRIC_AGG_OPS:
                raise ValueError(f"{label}: aggregation op '{op}' must be one of {sorted(_METRIC_AGG_OPS)}")
            column = agg.get("column")
            if op in _METRIC_AGG_OPS_NEEDING_COLUMN and not column:
                raise ValueError(f"{label}: aggregation op '{op}' requires a 'column'")
            entry = {"name": str(agg.get("name") or (op if not column else f"{op}_{_norm_ident(column)}")), "op": op}
            if column:
                entry["column"] = str(column)
            if op == "percentile":
                try:
                    q = float(agg.get("q", 0.5))
                except (TypeError, ValueError):
                    raise ValueError(f"{label}: percentile 'q' must be a number between 0 and 1") from None
                if not 0 < q <= 1:
                    raise ValueError(f"{label}: percentile 'q' must be between 0 and 1")
                entry["q"] = q
            if op == "rate":
                if agg.get("truthy") is not None:
                    if not isinstance(agg["truthy"], list) or not agg["truthy"]:
                        raise ValueError(f"{label}: rate 'truthy' must be a non-empty list")
                    entry["truthy"] = [str(v) for v in agg["truthy"]]
                if agg.get("denominator") is not None:
                    denom = str(agg["denominator"])
                    if denom not in ("rows", "nonblank"):
                        raise ValueError(f"{label}: rate 'denominator' must be 'rows' or 'nonblank'")
                    entry["denominator"] = denom
            norm_aggs.append(entry)
        spec["aggregations"] = norm_aggs
        if item.get("top_n") is not None:
            try:
                top_n = int(item["top_n"])
            except (TypeError, ValueError):
                raise ValueError(f"{label}: top_n must be a positive integer") from None
            if top_n < 1:
                raise ValueError(f"{label}: top_n must be a positive integer")
            spec["top_n"] = top_n
        if item.get("sort_desc") is not None:
            spec["sort_desc"] = bool(item["sort_desc"])
        out.append(spec)
    return out


def _load_metric_specs(raw: str) -> list[dict[str, Any]]:
    """Load metric specs from inline JSON or a JSON/YAML file path."""
    text = raw.strip()
    if text.startswith("{") or text.startswith("["):
        try:
            data = json.loads(text)
        except json.JSONDecodeError as e:
            raise ValueError(f"inline --metrics is not valid JSON: {e}") from None
    else:
        ...
        if path.suffix.lower() in (".yaml", ".yml"):
            ...
            try:
                ...
                data = yaml.safe_load(body)
    return _normalize_metric_specs(data)


_SQL_NUMERIC_HINTS = ("int", "real", "num", "float", "doub", "dec")
_TIME_UNIT_WORDS = {
...
_GROUP_PHRASE_RE = re.compile(
    r"\b(?:per|by|for each|each|across|grouped by|group by|broken down by)[\s\-]+([a-z0-9_][a-z0-9_ \-/]*)",
    re.I,
)
_TOP_N_RE = re.compile(r"\btop\s+(\d{1,4})\s+([a-z0-9_][a-z0-9_ \-]*)", re.I)
_PERCENTILE_RE = re.compile(r"\bp(\d{1,2})\b", re.I)
_ORDINAL_PERCENTILE_RE = re.compile(r"\b(\d{1,2})(?:st|nd|rd|th)\s+percentile\b", re.I)
_ERROR_RATE_RE = re.compile(r"\b(?:error|failure|fail)[\s\-_]*(?:rate|pct|percent|percentage|ratio|%)", re.I)
_CLAUSE_SPLIT_RE = re.compile(r"\b(?:by|per|with|for|and|of)\b", re.I)


def _columns_from_profile(profile: dict[str, Any]) -> tuple[list[str], set[str], str | None, str | None]:
    """Return (columns, numeric_columns, sqlite_table, skip_reason).

    Distinct counts are exposed separately by `_distinct_from_profile` because
    they are exact for table files but only sampled for SQLite.
    """
    if profile.get("input_type") == "sqlite":
        tables = [t for t in (profile.get("tables") or []) if int(t.get("row_count") or 0) > 0]
        if len(tables) != 1:
            detail = "no non-empty tables" if not tables else f"{len(tables)} candidate tables"
            return [], set(), None, f"metric inference skipped for SQLite input ({detail}); pass --metrics with a 'table'"
        table = tables[0]
        columns = [str(c.get("name")) for c in (table.get("columns") or []) if c.get("name")]
        numeric = {
            str(c["name"]) for c in (table.get("columns") or [])
            if c.get("name") and any(
                h in str(c.get("declared_type") or c.get("type") or "").lower()
                for h in _SQL_NUMERIC_HINTS
            )
        }
        # SQLite columns may be declared without a type; fall back to the sampled values.
        sampled = (table.get("sample_profile") or {}).get("columns") or []
        numeric |= {
            str(c["name"]) for c in sampled
            if c.get("name") and c.get("inferred_type") == "number"
        }
        return columns, numeric, str(table.get("name")), None
    columns = [str(c.get("name")) for c in (profile.get("columns") or []) if c.get("name")]
    numeric = {str(c["name"]) for c in (profile.get("columns") or []) if c.get("inferred_type") == "number"}
    if not columns:
        return [], set(), None, "metric inference skipped: input has no detected columns"
    return columns, numeric, None, None


def _distinct_from_profile(profile: dict[str, Any]) -> dict[str, int]:
    """Exact distinct counts per column, or {} when the profile only sampled."""
    if profile.get("input_type") == "sqlite":
        return {}  # sample_profile covers a handful of rows; not a cardinality signal
    return {
        str(c["name"]): int(c["distinct"])
...
def _detect_time_column(columns: list[str]) -> str | None:
    ...
    for col in columns:
        tokens = set(_ident_tokens(col))
        if tokens & {"ts", "timestamp", "datetime"}:
            ranked.append((0, col))
        elif tokens & {"time", "date"}:
            ranked.append((1, col))
        elif tokens & {"created", "occurred", "started", "logged"}:
            ranked.append((2, col))
    ranked.sort()
    return ranked[0][1] if ranked else None


def _detect_error_column(columns: list[str]) -> str | None:
    ranked: list[tuple[int, str]] = []
    for col in columns:
        tokens = set(_ident_tokens(col))
        if "error" in tokens:
            ranked.append((0 if "is" in tokens else 1, col))
        elif tokens & {"err", "failed", "failure", "fail"}:
            ranked.append((2, col))
    ranked.sort()
    return ranked[0][1] if ranked else None


_INFERRED_GROUP_CAP = 50


def _infer_metrics_from_question(
    question: str,
    columns: list[str],
    ...
    """Scaffold metric specs from the question text and the profiled columns.

    Deterministic and conservative: a dimension is only emitted when the phrase
    maps to a column that actually exists. Unmatched phrases are reported so the
    caller knows to supply --metrics.
    """
    notes: list[str] = []
    ...
    # Percentile levels requested anywhere in the question.
    qs: set[float] = set()
    for m in _PERCENTILE_RE.finditer(q):
        value = int(m.group(1))
        if 1 <= value <= 99:
            qs.add(round(value / 100.0, 2))
    for m in _ORDINAL_PERCENTILE_RE.finditer(q):
        value = int(m.group(1))
        if 1 <= value <= 99:
            qs.add(round(value / 100.0, 2))
    if re.search(r"\bmedian\b", q, re.I):
        qs.add(0.5)

    # Numeric columns named in the question.
    q_norm = "_" + _norm_ident(q) + "_"
    named_numeric = [c for c in columns if c in numeric_columns and f"_{_norm_ident(c)}_" in q_norm]

    wants_error_rate = bool(_ERROR_RATE_RE.search(q))
    error_column = _detect_error_column(columns) if wants_error_rate else None
    if wants_error_rate and not error_column:
        notes.append("question asks for an error rate but no error/failure column was found in the input")
    wants_mean = bool(re.search(r"\b(?:mean|average|avg)\b", q, re.I))
    wants_sum = bool(re.search(r"\b(?:sum|total)\b", q, re.I))

    # top N <dimension>
    top_n_by_column: dict[str, int] = {}
    for m in _TOP_N_RE.finditer(q):
        phrase = _CLAUSE_SPLIT_RE.split(m.group(2))[0].strip()
        col = _match_column(phrase, columns)
        if col:
            top_n_by_column[col] = int(m.group(1))
            matched[phrase] = col
        elif phrase:
    ...
    for m in _GROUP_PHRASE_RE.finditer(q):
        for raw_phrase in re.split(r"\band\b|/|,", m.group(1), flags=re.I):
            phrase = raw_phrase.strip(" -")
            if not phrase:
                continue
            unit = _TIME_UNIT_WORDS.get(_ident_tokens(phrase)[-1] if _ident_tokens(phrase) else "")
            if unit:
                time_col = _detect_time_column(columns)
                if time_col:
                    bucket = {"column": time_col, "unit": unit}
                    matched[phrase] = f"{time_col} ({unit} bucket)"
                else:
                    unmatched.append(phrase)
                    notes.append(f"'{phrase}' needs a timestamp column but none was detected")
                continue
            col = _match_column(phrase, columns)
            if col:
                ...
                unmatched.append(phrase)
    for col in top_n_by_column:
        if col not in dimensions:
            dimensions.append(col)

    def build_aggregations(grouped: bool) -> list[dict[str, Any]]:
        aggs: list[dict[str, Any]] = [{"name": "count", "op": "count"}]
        if grouped:
            aggs.append({"name": "share_pct", "op": "share"})
        if error_column:
            aggs.append({"name": "error_pct", "op": "rate", "column": error_column})
        for col in named_numeric:
            for q_level in sorted(qs):
                aggs.append({
                    "name": f"p{int(round(q_level * 100))}_{_norm_ident(col)}",
                    "op": "percentile",
                    "column": col,
                    "q": q_level,
                })
            if wants_mean:
                aggs.append({"name": f"mean_{_norm_ident(col)}", "op": "mean", "column": col})
            if wants_sum:
                aggs.append({"name": f"sum_{_norm_ident(col)}", "op": "sum", "column": col})
        return aggs

    has_signal = bool(dimensions or bucket or (named_numeric and (qs or wants_mean or wants_sum)) or error_column)
    if not has_signal:
        notes.append(
            "no group-by dimension, percentile, or rate could be derived from the question; "
            "pass --metrics to declare aggregations explicitly"
        )
        return [], {"source": "inferred", "matched": matched, "unmatched": sorted(set(unmatched)), "notes": notes}

    if named_numeric and not (qs or wants_mean or wants_sum):
        notes.append("numeric columns were named but no percentile/mean/sum was requested; only counts are computed")

    def decorate(spec: dict[str, Any]) -> dict[str, Any]:
        if table:
            spec["table"] = table
        if input_path:
    ...
    specs: list[dict[str, Any]] = [decorate({
        "name": "overall",
        "description": "Whole-dataset totals for the question's measures.",
        "group_by": [],
        "aggregations": build_aggregations(grouped=False),
    })]
    for col in dimensions:
        spec = decorate({
            "name": f"by_{_norm_ident(col)}",
            "description": f"Grouped by {col}.",
            "group_by": [col],
            "aggregations": build_aggregations(grouped=True),
            "sort_by": "count",
            "sort_desc": True,
        })
        if col in top_n_by_column:
            spec["top_n"] = top_n_by_column[col]
        else:
            cardinality = (distinct_counts or {}).get(col)
            if cardinality and cardinality > _INFERRED_GROUP_CAP:
                # Keep audit.md readable; group_count still reports the true total
                # and truncated_to records that rows were withheld.
                spec["top_n"] = _INFERRED_GROUP_CAP
                notes.append(
                    f"'{col}' has {cardinality} distinct values; the inferred metric shows the "
                    f"top {_INFERRED_GROUP_CAP} by count (set top_n via --metrics to change this)"
                )
        specs.append(spec)
    if bucket:
        bucket_label = f"{bucket['column']}_{bucket['unit']}"
        specs.append(decorate({
            "name": f"by_{_norm_ident(bucket_label)}",
            "description": f"Bucketed by {bucket['unit']} on {bucket['column']}.",
            "group_by": [],
            "bucket": bucket,
            "aggregations": build_aggregations(grouped=True),
            "sort_by": bucket_label,
            "sort_desc": False,
        }))

    notes.append("filters are not inferred; pass --metrics to scope a metric to a subset of rows")
    return specs, {
        "source": "inferred",
        "matched": matched,
...
def cmd_analyze_plan(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "metrics", None):
        try:
            explicit_metrics = _load_metric_specs(args.metrics)
        except (ValueError, yaml.YAMLError, OSError) as e:
            print(f"ERROR: invalid --metrics spec: {e}", file=sys.stderr)
            return 2
    ...
    if explicit_metrics:
        ...
        metric_inference: dict[str, Any] = {
            "source": "explicit",
            "notes": ["metrics supplied via --metrics; question inference was not run"],
        }
    elif args.no_infer_metrics:
        metrics = []
        metric_inference = {
            "source": "disabled",
            "notes": ["--no-infer-metrics was set; the generated script performs profiling only"],
        }
    else:
        primary = profiled_inputs[0]
        columns, numeric_columns, table, skip_reason = _columns_from_profile(primary["profile"])
        if skip_reason:
            metrics = []
            metric_inference = {"source": "inferred", "matched": {}, "unmatched": [], "notes": [skip_reason]}
        else:
            metrics, metric_inference = _infer_metrics_from_question(
                args.question,
                columns,
                numeric_columns,
                table=table,
                input_path=primary["path"] if len(profiled_inputs) > 1 else None,
                distinct_counts=_distinct_from_profile(primary["profile"]),
            )
            try:
                metrics = _normalize_metric_specs(metrics)
            except ValueError as e:  # pragma: no cover - inference builds valid specs
                print(f"ERROR: inferred metrics failed validation: {e}", file=sys.stderr)
                return 2
        if len(profiled_inputs) > 1:
            metric_inference.setdefault("notes", []).append(
                f"metrics were inferred from the first input only ({len(profiled_inputs)} inputs declared)"
            )

    if metrics:
        limitations.insert(0, (
            "Metric values are computed by the generated script from the declared specs; "
            "review group_by, filters, and denominators before quoting them."
        ))
        if metric_inference.get("source") == "inferred":
            limitations.insert(1, (
                "Metrics were scaffolded from the question text, not confirmed by a human; "
                "unmatched phrases are listed under metric_inference."
            ))
    else:
        limitations.insert(0, (
            "Generated script performs deterministic profiling only until metric formulas or SQL "
            "are added to the plan (see --metrics)."
        ))
    ...
    plan = {
        ...
        "validations": [
            ...
            "every declared metric must compute against its input (a missing column fails the run)",
        ],
        "certainty_rubric": {
    ...
    if metrics:
        print(f"Metrics:       {len(metrics)} ({metric_inference.get('source')}) — "
              + ", ".join(m["name"] for m in metrics))
    else:
        print("Metrics:       none — profiling only")
    for phrase in metric_inference.get("unmatched") or []:
        print(f"  unmatched question phrase: '{phrase}'")
    for note in metric_inference.get("notes") or []:
        print(f"  note: {note}")
...
def main() -> int:
    ...
    sp.add_argument(
        "--metrics",
        help="Metric specs as inline JSON or a path to a JSON/YAML file; overrides question inference",
    )
    sp.add_argument(
        "--no-infer-metrics",
        action="store_true",
        help="Skip question-driven metric scaffolding (profiling only)",
    )
