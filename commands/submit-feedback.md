...
description: Report a bug or request a feature for the research plugin — drafts a GitHub issue and files it only after you approve
argument-hint: "[what broke, or what you wish it did]"
---

# Submit research feedback

File the user's report as a GitHub issue on `tyroneross/research-plugin`. Issues are this
...
3. **Draft the issue.** Use the template for the classified kind:
   ...
   **Bug**
   ...
   ## Steps to reproduce
   ## Environment
   plugin: research-plugin @ <sha> · claude: <version> · os: <uname -sm>
   ```
...
5. **File it**, only after approval:
   ...
   ```bash
   gh issue create --repo tyroneross/research-plugin \
...
6. **If `gh` is missing or unauthenticated**, do not fail — print the URL so the user
   ...
   https://github.com/tyroneross/research-plugin/issues/new
